...
# Details listed in documentation
# [Link]
...
#- pyLaTeX Functions
from pylatex import Document as __Document, Section as __Section, \
    Alignat as __Alignat, Center as __Center
from os import startfile as __startfile


def __texout(width, height, text):
    geometry_options = {"tmargin": "1cm", "lmargin": "10cm",
                        "paperwidth": str(width) + "pt", "paperheight": str(height) + "pt",
                        "left": "40pt", "top": "40pt"}

    doc = __Document(geometry_options=geometry_options)

    with doc.create(__Section("")):
        with doc.create(__Alignat(numbering=False, escape=False)) as agn:
            __Center().create(agn.append(text))

    doc.generate_tex("full")
    __startfile("full.tex")


def contfractex(inp, a = 0):
    # Notice the fact that protoVal is reversed typewise relative to inp input
    protoVal = contfrac(inp, a)
    ...
    __texout(16383, 16383, text)

def recfractex(n):
    patArr = r"\frac{}{}"

    for i in range(n - 1):
        patArr = r"\frac{" + patArr + "}" + "{" + patArr + "}"

    proto_start_text1 = r"N_{"
    proto_start_text1 += str(n)
    proto_start_text2 = "} &= "
    start_text = proto_start_text1 + proto_start_text2
    ...
    for i in range(len(patArr)):
        text += patArr[i]
        if patArr[i] == "{" and patArr[i + 1] == "}":
            text += str(num + patNumArr[countNum])
            text += "+"
            countNum += 1
            text += str(num + patNumArr[countNum])
            countNum += 1
